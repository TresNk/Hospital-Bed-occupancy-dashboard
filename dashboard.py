...
if 'results' not in st.session_state:
    ...
    # Create tabs
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Performance Overview", "System Dynamics", "Sensitivity Analysis", "Raw Data Log", "Scenario Analysis"])
    ...
    with tab2:
        ...
        st.markdown("These figures were found in the `report` directory. Since 'Figure 4.1' could not be identified from `introduction.tex`, both are displayed here.")

        col1, col2 = st.columns(2)

        with col1:
            st.image(
                "report/blocking_probability.png",
                caption="Analysis of Blocking Probability",
                use_column_width=True
            )

        with col2:
            st.image(
                "report/wait_time.png",
                caption="Analysis of Patient Wait Times",
                use_column_width=True
            )
    ...
    with tab5:
        st.header("Scenario Analysis: Blocking Probability vs. System Utilization")
        if 'comparison_runs' in st.session_state and len(st.session_state.comparison_runs) > 0:
            comparison_df = pd.DataFrame.from_dict(st.session_state.comparison_runs, orient='index')
            df_plot = comparison_df.reset_index().rename(columns={'index': 'Scenario'})
            
            kpis_to_plot = ['Blocking Probability (%)', 'Average Bed Occupancy (%)']
            kpis_present = [kpi for kpi in kpis_to_plot if kpi in df_plot.columns]
            
            if len(kpis_present) > 0:
                df_melted = df_plot.melt(id_vars='Scenario', value_vars=kpis_present, var_name='Metric', value_name='Value')
                
                # Rename for clarity in the plot
                df_melted['Metric'] = df_melted['Metric'].replace({'Average Bed Occupancy (%)': 'System Utilization (%)'})

                fig_specific_kpis = px.bar(
                    df_melted, 
                    x="Scenario", 
                    y="Value", 
                    color="Metric", 
                    barmode="group",
                    title="Blocking Probability vs. System Utilization"
                )
                st.plotly_chart(fig_specific_kpis, use_container_width=True)
                
                st.write("**Full Comparison Results:**")
                st.dataframe(comparison_df.style.format("{:.2f}"))
            else:
                st.warning("The required KPIs ('Blocking Probability (%)', 'Average Bed Occupancy (%)') are not available in the comparison data.")
        else:
            st.info("Run a simulation and click 'Add to Comparison' on the 'Performance Overview' tab to build the analysis chart.")
